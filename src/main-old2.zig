...
const RT = struct {
    ...
    fn eql(rt: *RT, aref: Ref, bref: Ref) bool {
        ...
        if (aref._property != bref._property) return false; // simultaneous kind and hash test
...
fn benchmark() !void {
    ...
    const m = 30000;
    std.debug.print("cap\tns\tt_gc\tt_fnd\tt_crt\tn_ass\tn_diss\tn_creat\n", .{});
    ...
    for (ns) |n| {
        ...
        var to_search = try std.ArrayList(Ref).initCapacity(alloc, m);
        ...
        for (0..m) |_k| {
            ...
            if (rt.hamtContains(h, a)) acc += k * rt.get(.real, rt.hamtGet(h, a));
        }
        ...
        std.debug.print(
            "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t\t{}\n",
            .{ n, t_edit / m, t_gc / m, t_find / m, t_create / m, n_assoc, n_dissoc, rt.gc.n_created, acc },
        );
    }
}
