...
fn scratch() !void {
    ...
    const a = rt.newReal(76);
    const b = rt.newReal(438);
    const c = rt.newReal(136);

    var h = rt.newHamt();
    debugPrint(h);
    h = rt.hamtAssoc(h, a, a);
    debugPrint(h);
    h = rt.hamtAssoc(h, b, b);
    debugPrint(h);
    h = rt.hamtAssoc(h, c, c);
    debugPrint(h);
    std.debug.print("\n", .{});
    ...
    var it = rt.hamtIter(h);
    while (it.next()) |objkv| debugPrint(objkv);
    std.debug.print("\n", .{});
...
fn fuzz() !void {
    ...
    for (ns) |n| {
        ...
        for (0..m) |i| {
            ...
            // debugPrint(h);
            // std.debug.print("key = {} ({} {})\n", .{ x, rt.hamtContains(h, a), s.contains(x) });
